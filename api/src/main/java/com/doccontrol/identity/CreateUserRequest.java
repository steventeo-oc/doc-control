 ...
 * and every membership states its level explicitly — an omitted level is a
 * validation error, never an assumption (plan-back F1). Roles default to
 * ["User"] when omitted.
 */
public record CreateUserRequest(
        @NotBlank @Size(max = 255) String name,
        @NotBlank @Email @Size(max = 255) String email,
        @NotEmpty List<@Valid MembershipInput> departments,
